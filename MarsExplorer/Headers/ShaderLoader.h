...
inline unsigned int loadShader(const std::string &vertex_shader_file, const std::string &fragment_shader_file) {
...
//returneaza id-ul programului shader OpenGL
inline unsigned int loadShader(const std::string &vertex_shader_file, const std::string &geometry_shader_file, const std::string &fragment_shader_file) {
...
inline unsigned int _createShader(const std::string &shader_file, GLenum shader_type) {
  ...
  if (compile_result == GL_FALSE) {
    ...
    glGetShaderInfoLog(gl_shader_object, info_log_length, nullptr, &shader_log[0]);
...
inline unsigned int _createProgram(const std::vector<unsigned int> &shader_objects) {
  ...
  if (link_result == GL_FALSE) {
    ...
    glGetProgramInfoLog(gl_program_object, info_log_length, nullptr, &program_log[0]);
  ...
  for (auto it = shader_objects.begin(); it != shader_objects.end(); ++it) glDeleteShader((*it));
